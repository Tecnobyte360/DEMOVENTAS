...
<div x-data="{ deleteOpen:false }" class="p-6 md:p-8 space-y-6">
  ...
  {{-- ================= HEADER / HERO ================= --}}
  <section class="relative overflow-hidden rounded-3xl bg-gradient-to-r from-indigo-600 via-violet-600 to-fuchsia-600 text-white shadow-2xl">
    <div class="px-6 md:px-8 py-8 md:py-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
      <div class="space-y-1">
        ...
        <p class="text-sm text-white/80">Consulta, filtra y analiza todos los pagos asociados a facturas emitidas.</p>
      </div>
    ...
    <section class="p-6 md:p-8 border-b border-gray-100 dark:border-gray-800" aria-label="Filtros de búsqueda">
      <header class="mb-4">
        <h2 class="text-lg md:text-xl font-bold text-gray-900 dark:text-white flex items-center gap-3">
          <span class="inline-grid place-items-center w-8 h-8 md:w-9 md:h-9 rounded-xl bg-indigo-100 text-indigo-700 dark:bg-indigo-900/40 dark:text-indigo-200">
      ...
      <div class="grid grid-cols-1 md:grid-cols-12 gap-4">
        ...
        <div class="md:col-span-4">
          ...
          <input type="text" wire:model.debounce.400ms="buscar"
                 ...
                 class="w-full h-12 px-4 rounded-2xl border-2 border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 dark:text-white focus:outline-none focus:ring-4 focus:ring-indigo-300/60">
        </div>
        ...
        {{-- Medio de pago --}}
        <div class="md:col-span-2">
          <label class="block text-xs font-semibold uppercase tracking-wider text-gray-600 dark:text-gray-300 mb-2">Medio de pago</label>
        ...
        <div class="md:col-span-2 flex items-end gap-3">
          <button wire:click="$refresh"
                  class="flex items-center gap-2 px-4 py-2 rounded-2xl bg-gradient-to-r from-indigo-600 to-violet-600 hover:from-indigo-700 hover:to-violet-700 text-white shadow">
            <i class="fa-solid fa-magnifying-glass"></i> Buscar
          </button>
          <button wire:click="$set('buscar','');$set('medio_pago_id',null);$set('fecha_inicio',null);$set('fecha_fin',null)"
        ...
        @forelse($pagos as $p)
          <article class="rounded-2xl border border-gray-200 dark:border-gray-800 p-4 bg-white dark:bg-gray-900">
            <div class="flex justify-between items-start mb-2">
              <span class="text-sm font-semibold text-indigo-700 dark:text-indigo-300">{{ $p->metodo ?: '—' }}</span>
      ...
      <div class="hidden md:block overflow-x-auto rounded-2xl border border-gray-200 dark:border-gray-800">
        <table class="min-w-full text-sm">
          ...
          <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
            ...
            @empty
              <tr>
                <td colspan="7" class="p-6 text-center text-gray-500 dark:text-gray-400">No hay pagos registrados.</td>
              </tr>
            @endforelse
          </tbody>
        </table>
  ...
  <div wire:loading.delay
       class="fixed inset-x-0 bottom-4 mx-auto w-fit px-4 py-2 rounded-full bg-indigo-600 text-white shadow-lg">
