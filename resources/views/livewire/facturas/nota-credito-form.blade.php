...
<section
    class="mt-6 md:mt-8 rounded-3xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 shadow-2xl overflow-hidden relative">

    {{-- ===== MARCA DE AGUA SEGÚN ESTADO ===== --}}
    <div x-data="{ estado: @entangle('estado') }" x-show="['emitida','anulada','cerrado'].includes(estado)" class="doc-watermark">
    ...
    <section class="p-6 md:p-8 border-b border-gray-100 dark:border-gray-800" aria-label="Datos de la nota crédito">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <section>
                ...
                <select wire:key="cliente-select-{{ $socio_negocio_id ?? 'x' }}"
            ...
            <section>
                <label
                    class="block text-xs font-semibold uppercase tracking-wider text-gray-600 dark:text-gray-300 mb-2">Serie
                    (NC)</label>
            ...
            <section class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    <label
                        class="block text-xs font-semibold uppercase tracking-wider text-gray-600 dark:text-gray-300 mb-2">Motivo</label>
                    <input type="text" wire:model.debounce.300ms="motivo"
        ...
        <div class="mt-6 grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <section>
                <label
                    class="block text-xs font-semibold uppercase tracking-wider text-gray-600 dark:text-gray-300 mb-2">Factura
                    a afectar (opcional)</label>
                ...
                <div class="mb-2 hidden" wire:loading.class.remove="hidden" wire:target="socio_negocio_id">
                    <span
                        class="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 text-xs">
                        <i class="fa-solid fa-spinner fa-spin"></i> Cargando facturas del cliente…
                    </span>
                </div>

                <select wire:model.number="factura_id"
                    wire:key="facturas-select-{{ $socio_negocio_id ?? 0 }}-{{ count($facturasCliente) }}"
                    wire:change="onFacturaElegida($event.target.value)" @disabled(empty($socio_negocio_id))
            ...
            <section>
                ...
                @php
                    ...
                    $opActual =
                        !$tieneSel && $selectedId ? \App\Models\CuentasContables\PlanCuentas::find($selectedId) : null;
                @endphp

                <select wire:model.number="cuenta_cobro_id"
                    class="w-full h-12 md:h-14 px-4 rounded-2xl border-2 border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 dark:text-white focus:outline-none focus:ring-4 focus:ring-violet-300/60">
                    @if ($opActual)
            ...
            <section>
                ...
                <input type="text" placeholder="Observaciones visibles en el documento"
                    wire:model.debounce.400ms="notas"
                    class="w-full h-12 md:h-14 px-4 rounded-2xl border-2 border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 dark:text-white focus:outline-none focus:ring-4 focus:ring-violet-300/60">
            </section>
        </div>
    </section>
    ...
    <section class="p-6 md:p-8" aria-label="Líneas de la nota crédito">
        <section class="rounded-2xl border border-gray-200 dark:border-gray-800 overflow-x-auto shadow"
            aria-label="Tabla de líneas" {{-- ✅ key alto nivel para reconstruir tabla cuando cambia factura o cantidad de líneas --}}
            ...
            <table class="min-w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-800" {{-- ✅ key del tbody para forzar diff limpio en cambios de factura/lineas --}}
                    ...
                    @forelse($lineas as $i => $l)
                        ...
                        <tr wire:key="linea-{{ $i }}-{{ $lineas[$i]['producto_id'] ?? 'x' }}-{{ $lineas[$i]['bodega_id'] ?? 'x' }}"
                            ...
                            <td class="px-4 py-3 min-w-[260px]">
                                <select wire:key="prod-{{ $i }}-{{ $lineas[$i]['producto_id'] ?? 'x' }}"
                            ...
                            <td class="px-4 py-3 min-w-[260px]">
                                ...
                                <div class="space-y-1">
                                    <select wire:model.number="lineas.{{ $i }}.cuenta_ingreso_id"
                            ...
                            <td class="px-4 py-3">
                                <input type="text" placeholder="Descripción (opcional)"
                                    wire:model.debounce.250ms="lineas.{{ $i }}.descripcion"
                                    class="w-full h-12 px-3 rounded-xl border-2 border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 dark:text-white focus:outline-none focus:ring-4 focus:ring-violet-300/60">
                            </td>
                            ...
                            <td class="px-4 py-3 min-w-[240px]">
                                <select wire:model.number="lineas.{{ $i }}.impuesto_id"
                            ...
                            <td class="px-4 py-3 text-right">
                                <div class="inline-flex items-center gap-2">
                                    <button type="button" wire:click="addLinea"
                                        ...
                                        wire:target="addLinea,removeLinea,guardar,emitir,abrirAplicacion">
                                        + Línea
                                    </button>
                ...
                <tfoot class="bg-gray-50 dark:bg-gray-800/40">
                    ...
                    <tr>
                        <td colspan="10"></td>
                        <td class="px-4 py-2 text-right font-medium text-gray-700 dark:text-gray-300">Impuestos NC:
                        </td>
                        <td class="px-4 py-2 text-right font-semibold text-gray-900 dark:text-gray-100">
    ...
    @php
        ...
        $saldoAplicar = round((float) ($this->total ?? 0), 2);
        $bloqueaEmitir = false;
    @endphp
        ...
        <div class="px-4 md:px-8 py-4">
            <div class="grid grid-cols-1 xl:grid-cols-12 gap-4 items-center">
                ...
                <div
                    ...
                    @if ($factura_id)
                        <span
                            class="inline-flex items-center gap-2 px-2 py-1.5 rounded-full bg-indigo-100 text-indigo-700 dark:bg-indigo-900/40 dark:text-indigo-200 text-xs">
                    ...
                    <div x-data="{
                        ...
                        <div class="flex flex-wrap justify-end gap-2">
                            ...
                            <button type="button"
                                ...
                                wire:click="emitir" wire:loading.attr="disabled" wire:target="emitir,guardar"
                                x-data="{ e: @entangle('estado') }" :disabled="['anulada', 'cerrado'].includes(e)">
