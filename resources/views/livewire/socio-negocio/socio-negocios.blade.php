...
<div x-data="{
    ...
    <section
        class="relative overflow-hidden rounded-3xl bg-gradient-to-r from-gray-100 via-gray-200 to-gray-300 text-gray-900 shadow-xl">
        <div
            class="px-6 md:px-8 py-8 md:py-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">

            <!-- Título -->
            <div class="space-y-1">
                <h1 class="text-2xl md:text-3xl font-extrabold tracking-tight flex items-center gap-3">
                    <span
                        class="inline-flex items-center justify-center w-12 h-12 rounded-2xl bg-white/60 shadow-sm backdrop-blur">
            ...
            <div class="flex flex-wrap items-center gap-2">
                ...
                <span
                    class="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/50 shadow-sm text-gray-700 font-semibold text-xs md:text-sm">
    ...
    <section
        ...
        <header class="px-6 md:px-8 py-4 border-b border-gray-100 dark:border-gray-800">
            <h2 class="text-lg md:text-xl font-bold text-gray-900 dark:text-white flex items-center gap-3">
                <span
                    class="inline-grid place-items-center w-8 h-8 md:w-9 md:h-9 rounded-xl bg-violet-100 text-violet-700 dark:bg-violet-900/40 dark:text-violet-200">
        ...
        <div class="px-6 md:px-8 py-6 grid grid-cols-1 lg:grid-cols-12 gap-4">
            ...
            <div class="lg:col-span-6">
                <label
                    class="block text-xs font-semibold uppercase tracking-wider text-gray-600 dark:text-gray-300 mb-2">Ir
                    a un socio puntual</label>
                <div wire:ignore x-data x-init="const t = new TomSelect($refs.selSocio, {
            ...
            <div class="lg:col-span-6">
                <label
                    class="block text-xs font-semibold uppercase tracking-wider text-gray-600 dark:text-gray-300 mb-2">Buscar
                    (razón social, NIT, dirección)</label>
                <div class="flex gap-2">
                    <input type="text" wire:model.debounce.500ms="buscador" placeholder="Escribe para filtrar…"
                        class="w-full h-12 px-4 rounded-2xl border-2 border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 dark:text-white">
    ...
    <section
        ...
        <header
            ...
            <div class="flex items-center gap-3">
                <span
                    class="inline-grid place-items-center w-9 h-9 rounded-xl bg-violet-100 text-violet-700 dark:bg-violet-900/40 dark:text-violet-200">
                    <i class="fas fa-rectangle-list"></i>
                </span>
                <h3 class="text-lg font-bold text-gray-800 dark:text-white">Todos los socios</h3>
            </div>
            <span
                class="text-xs px-2 py-1 rounded-full bg-violet-100 text-violet-700 dark:bg-violet-900/30 dark:text-violet-300">
                {{ $socios->count() }}
            </span>
        </header>
                ...
                @forelse($socios as $s)
                    @php
                        ...
                        $cp = is_array($s->condiciones_pago_efectivas ?? null)
                            ...
                            : (is_array($s->condiciones_pago ?? null)
                                ? $s->condiciones_pago
                                : []);
                    @endphp
                    <article
                        ...
                        <div class="flex items-start justify-between gap-3">
                            <div>
                                <div class="flex items-center gap-2">
                                    <span
                                        class="inline-flex items-center justify-center text-[10px] px-2 py-0.5 rounded-full
                               {{ $esCliente ? 'bg-emerald-100 text-emerald-700' : 'bg-sky-100 text-sky-700' }}">
                                ...
                                <div class="mt-2 flex flex-wrap gap-1">
                                    ...
                                    @if (!empty($cp))
                                        ...
                                        @if ($tipo)
                                            <span
                                                class="text-[10px] px-2 py-0.5 rounded-full bg-indigo-100 text-indigo-700">{{ $tipo }}</span>
                                        @endif
                                        @if (!empty($cp['plazo_dias']))
                                            <span
                                                class="text-[10px] px-2 py-0.5 rounded-full bg-blue-100 text-blue-700">{{ $cp['plazo_dias'] }}
                                                días</span>
                                        @endif
                                        @if (array_key_exists('interes_mora_pct', $cp) && $cp['interes_mora_pct'] !== null && $cp['interes_mora_pct'] !== '')
                                            <span
                                                class="text-[10px] px-2 py-0.5 rounded-full bg-rose-100 text-rose-700">
                                                Mora: {{ number_format((float) $cp['interes_mora_pct'], 2) }}%
                                            </span>
                                        @endif
                                        @if (!empty($cp['lista_precios']))
                                            <span
                                                class="text-[10px] px-2 py-0.5 rounded-full bg-teal-100 text-teal-700">
                                    ...
                                    @if (optional($s->dirEntregaPrincipal)->direccion)
                                        <span class="text-[10px] px-2 py-0.5 rounded-full bg-sky-100 text-sky-700">
                                            Entrega:
                                            {{ \Illuminate\Support\Str::limit(optional($s->dirEntregaPrincipal)->direccion, 30) }}
                                        </span>
                                    @endif
                                </div>
                            </div>
                            ...
                            <div class="text-right">
                                ...
                                <div
                                    class="text-sm font-extrabold {{ $esCliente ? 'text-emerald-600' : 'text-sky-600' }}">
            ...
            <div class="hidden md:block overflow-x-auto rounded-2xl border border-gray-200 dark:border-gray-800">
                <table class="min-w-full text-sm">
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                        @forelse($socios as $socio)
                            @php
                                ...
                                $cp = is_array($socio->condiciones_pago_efectivas ?? null)
                                    ...
                                    : (is_array($socio->condiciones_pago ?? null)
                            ...
                            <tr class="hover:bg-violet-50/50 dark:hover:bg-gray-800 transition">
                               ...
                               {{ $esCliente ? 'bg-emerald-100 text-emerald-700' : 'bg-sky-100 text-sky-700' }}">
                                ...
                                <td class="p-3">
                                    ...
                                    <div class="mt-1 flex flex-wrap gap-1">
                                        ...
                                        @if (optional($socio->dirEntregaPrincipal)->direccion)
                                            <span class="text-[10px] px-2 py-0.5 rounded-full bg-sky-100 text-sky-700">
                                                Entrega:
                                                {{ \Illuminate\Support\Str::limit(optional($socio->dirEntregaPrincipal)->direccion, 30) }}
                                            </span>
                                        @endif
                                    </div>
                                </td>
                                ...
                                <td
                                    class="p-3 text-center font-bold {{ $esCliente ? 'text-emerald-600' : 'text-sky-600' }}">
                                ...
                                <td class="p-3 text-center">
                                    @if ($esCliente)
                                        <button wire:click="mostrarPedidos({{ $socio->id }})"
                        ...
                        @empty
                            <tr>
                                <td colspan="10" class="p-6 text-center text-gray-500 dark:text-gray-400">Sin
                                    resultados</td>
                            </tr>
                        @endforelse
                    </tbody>
    ...
    <div class="fixed inset-0 z-50 grid place-items-center bg-black/50 backdrop-blur-sm"
        x-show="$wire.showCuentasModal" x-cloak @keydown.escape.window="$wire.cerrarCuentasModal()"
        @click.self="$wire.cerrarCuentasModal()">
        <div
            class="w-full max-w-3xl mx-4 rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 shadow-2xl overflow-hidden">
            <div
                class="px-6 py-4 border-b border-gray-200 dark:border-gray-800 flex items-center justify-between sticky top-0 bg-inherit">
                <h3 class="text-lg font-bold flex items-center gap-2">
                    <i class="fa-solid fa-book text-violet-600"></i> Asociar cuentas contables
                </h3>
                <button class="text-red-600" @click="$wire.cerrarCuentasModal()"><i
            ...
            <div class="p-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    @foreach ($campos as $c)
                        <div wire:ignore x-data x-init="const opts = @js($cuentasOptions);
            ...
            <div
                class="px-6 py-4 border-t border-gray-200 dark:border-gray-800 bg-gray-50/60 dark:bg-gray-800/50 flex justify-end gap-3 sticky bottom-0">
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-2 sm:p-4"
        ...
        <div class="w-[96vw] max-w-6xl xl:max-w-7xl rounded-2xl border border-gray-200 dark:border-gray-800
           bg-white dark:bg-gray-900 shadow-2xl overflow-hidden"
            @click.outside="showCreateModal=false">
            <!-- Header -->
            <div
                class="px-5 sm:px-6 py-4 border-b border-gray-200 dark:border-gray-800 flex items-center justify-between">
                <div class="flex items-center gap-3">
                    <span
                        class="inline-flex h-9 w-9 items-center justify-center rounded-xl bg-indigo-600/10 text-indigo-600">
                        <i class="fas fa-user-plus"></i>
                    </span>
                    <div>
                        <h3 class="text-base sm:text-lg font-extrabold text-gray-900 dark:text-white">Crear socio</h3>
                        <p class="text-xs sm:text-sm text-gray-500 dark:text-gray-400">Completa los datos para
                            registrar el socio de negocio.</p>
                    </div>
                </div>
               ...
               text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 transition"
                    @click="showCreateModal=false" aria-label="Cerrar">
                    <i class="fas fa-times"></i>
                </button>
            </div>

            <!-- Body (scroll aquí, no en todo el modal) -->
            <div class="p-4 sm:p-6 max-h-[82vh] overflow-y-auto">
    ...
    {{-- ================= MODAL: Importar ================= --}}
    <div class="fixed inset-0 z-50 grid place-items-center bg-black/50 backdrop-blur-sm" x-show="showImportModal"
        x-cloak @keydown.escape.window="showImportModal=false">
        <div
            class="w-full max-w-3xl mx-4 rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 shadow-2xl overflow-auto max-h-[85vh]">
            <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-800 flex items-center justify-between">
                <h3 class="text-lg font-bold">Importar socios de negocio</h3>
                <button class="text-red-600" @click="showImportModal=false"><i
                        class="fas fa-times-circle"></i></button>
            </div>
    ...
    {{-- ================= MODAL: Editar ================= --}}
    <div class="fixed inset-0 z-50 grid place-items-center bg-black/50 backdrop-blur-sm" x-show="showEditModal"
        x-cloak @keydown.escape.window="showEditModal=false">
        <div
            class="w-full max-w-4xl mx-4 rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 shadow-2xl overflow-auto max-h-[90vh]">
            <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-800 flex items-center justify-between">
                <h3 class="text-lg font-bold">Editar socio</h3>
                <button class="text-red-600" @click="showEditModal=false"><i
                        class="fas fa-times-circle"></i></button>
            </div>
    ...
    {{-- ================= MODAL: Pedidos ================= --}}
    <div class="fixed inset-0 z-50 grid place-items-center bg-black/60 backdrop-blur-sm" x-show="showPedidosModal"
        x-cloak @keydown.escape.window="showPedidosModal=false; @this.cerrarPedidosModal()"
        @click.self="showPedidosModal=false; @this.cerrarPedidosModal()">
        <div
            class="w-full max-w-4xl mx-4 rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 shadow-2xl overflow-auto max-h-[85vh]">
            <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-800 flex items-center justify-between">
                <h3 class="text-lg font-bold flex items-center gap-2">
            ...
            <div class="p-6">
                <div x-show="tab==='list'">
                    @if (empty($pedidosSocio))
                        <p class="text-gray-500 dark:text-gray-400 italic">No hay pedidos a crédito con saldo
                            pendiente.</p>
                    @else
                                ...
                                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                                    @foreach ($pedidosSocio as $p)
                                        <tr>
                                            ...
                                            <td class="px-4 py-2 text-right font-semibold dark:text-emerald-400">
                                                ${{ $p['total'] }}</td>
                                            <td class="px-4 py-2 text-center capitalize">{{ $p['tipo_pago'] }}</td>
                                            <td class="px-4 py-2 text-center">
                                                <button wire:click="mostrarDetallePedido({{ $p['id'] }})"
                                                    @click="tab='detail'"
                ...
                <div x-show="tab==='detail'" x-cloak class="space-y-4">
                    @if (!$mostrarDetalleModal)
                        <p class="text-gray-500 dark:text-gray-400 italic">Selecciona un pedido para ver el detalle.
                        </p>
                    @else
                        <div class="overflow-x-auto rounded-2xl border border-gray-200 dark:border-gray-700">
                            <table class="min-w-full text-sm">
                        ...
                        <div class="flex justify-between">
                            <button class="px-3 py-2 rounded-2xl border-2 border-gray-200 dark:border-gray-700"
                                @click="tab='list'">← Volver</button>
