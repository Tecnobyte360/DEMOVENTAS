...
<div x-data="{
    ...
    <section
        ...
        <section class="p-6 md:pb-2 md:px-8 border-b border-gray-100 dark:border-gray-800"
            ...
            <header class="mb-6">
                <div class="flex items-center justify-between gap-4">
                    ...
                    @if ($preview)
                        <span
                            class="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-200 text-xs">
                            <i class="fa-regular fa-eye" aria-hidden="true"></i> Próximo:
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <section>
                    ...
                    <select wire:model.live.number="socio_negocio_id" wire:change="onClienteChange($event.target.value)"
                        ...
                        @foreach ($clientes as $c)
                            <option value="{{ $c->id }}">{{ $c->razon_social }} @if ($c->nit)
                                    ({{ $c->nit }})
                                @endif
                            </option>
                        @endforeach
                        ...
                        <select wire:model.live="serie_id"
                            ...
                            @foreach ($series as $s)
                                <option value="{{ $s->id }}">
                                    ...
                                    @if ($s->prefijo)
                                        ({{ $s->prefijo }}:
                                        {{ str_pad($s->proximo, $s->longitud ?? 6, '0', STR_PAD_LEFT) }} →
                                        {{ $s->hasta }})
                                    @endif
                                    @if ($s->es_default)
                                        ★
                ...
                <section>
                    <label
                        class="block text-xs font-semibold uppercase tracking-wider text-gray-600 dark:text-gray-300 mb-2">Moneda</label>
                    <input type="text" wire:model.live="moneda"
                    ...
                    <div class="space-y-2">
                        <select wire:model.live="condicion_pago_id"
                            ...
                            @foreach ($condicionesPago as $cp)
                                <option value="{{ $cp->id }}">{{ $cp->nombre }} @if (!is_null($cp->dias))
                                        ({{ $cp->dias }} días)
                                    @endif
                                </option>
                            @endforeach
                        </select>
                        @error('condicion_pago_id')
                        ...
                        <div class="flex items-center gap-3">
                            <div class="flex-1">
                                <label
                                    class="block text-xs font-semibold uppercase tracking-wider text-gray-600 dark:text-gray-300 mb-1">Plazo
                                    (días)</label>
                                <input type="number" readonly value="{{ (int) ($plazo_dias ?? 0) }}"
                            ...
                            <div class="mt-6">
                                <span
                                    class="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-200 dark:bg-emerald-900/30 dark:text-emerald-200 dark:border-emerald-700 text-xs">
                                    <i class="fa-regular fa-clock" aria-hidden="true"></i>
                                    Vence en {{ (int) ($plazo_dias ?? 0) }}
                                    {{ (int) ($plazo_dias ?? 0) === 1 ? 'día' : 'días' }}
                                </span>
                            </div>
                        </div>
                    </div>
            ...
            <div class="mt-6 grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <section>
                    ...
                    <select wire:key="cxp-select-{{ (int) ($socio_negocio_id ?? 0) }}"
        ...
        <section class="p-6 md:p-8" aria-label="Líneas de la factura">
            <header class="mb-4 md:mb-6">
                ...
                {{ !$pid || !$bid
                    ? 'bg-gray-100 text-gray-500 border border-dashed border-gray-300 dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600'
                    : ($stock > 0
                        ? 'bg-green-50 text-green-700 border border-green-300'
                        : 'bg-red-50 text-red-700 border border-red-300') }}">
                            <i class="fas {{ !$pid || !$bid ? 'fa-info-circle' : ($stock > 0 ? 'fa-check-circle' : 'fa-exclamation-circle') }}"
                                aria-hidden="true"></i>
                            <span class="text-sm">
                                @if (!$pid || !$bid)
                                    Selecciona <strong>producto</strong> y <strong>bodega</strong> en la línea
                                    {{ $iStock + 1 }} para consultar stock.
                                @else
                                    Stock disponible (Línea {{ $iStock + 1 }}):
                                    <strong>{{ number_format($stock, 0) }}</strong>
                                @endif
                            </span>
                        </div>
                    </div>
                                            ...
                                            <div class="h-full w-full grid place-items-center">
                                                <svg viewBox="0 0 24 24" class="h-6 w-6 text-gray-400"
                                                    ...
                                                    <path fill="currentColor"
                                                        d="M21 19V5a2 2 0 0 0-2-2H5a2 2 0 0 0-2 2v14l4-4h12l2 2Zm-5-9a2 2 0 1 1-4.001-.001A 2 2 0 0 1 16 10Z" />
                                                </svg>
                                            </div>
                                        @endif
                                    </div>
            ...
            <div class="px-4 md:px-8 py-4">
                <div class="grid grid-cols-1 xl:grid-cols-12 gap-4 items-center">
                    ...
                    <div class="xl:col-span-3">
                        <div x-data="{ estado: @entangle('estado') }" class="flex flex-wrap justify-end gap-2">
                            {{-- Guardar --}}
                            <button type="button"
