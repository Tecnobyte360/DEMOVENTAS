...
class EntradasMercancia extends Component
{
    ...
    public string $concepto_rol = 'inventario'; // inventario|gasto|costo|ingreso|gasto_devolucion|ingreso_devolucion
   ...
   public function mount(?int $id = null): void
{
    ...
    $this->productos = Producto::where('activo', 1)
        ->orderBy('nombre')->take(800)->get(['id','nombre']);
    ...
    /** Sincroniza descripción y texto de cuenta al cambiar productos/bodegas/precios */
...
private function actualizarCuentaStr(int $i): void
{
    ...
    // 1) Prioriza cuenta de inventario por producto (producto_cuentas.tipo_id = inventario)
    $pid = (int)($this->entradas[$i]['producto_id'] ?? 0);
    if ($pid > 0) {
        $pc = ProductoCuenta::query()
            ->with('cuentaPUC:id,codigo,nombre')
            ->where('producto_id', $pid)
            ->where('tipo_id', (int)config('conta.tipo_inventario_id', 1)) // <-- define esto en config
            ->first();
    ...
    // 2) Si no hay cuenta por producto, cae a la cuenta del Concepto + Rol (primera por prioridad)
    if ($this->concepto_documento_id) {
        ...
        if ($c) {
            ...
            if ($match?->plan_cuenta_id) {
                ...
                if ($puc) $this->entradas[$i]['cuenta_str'] = "{$puc->codigo} — {$puc->nombre}";
            }
        }
    }
}
    ...
    public function emitirEntrada(): void
    {
        try {
            ...
            EntradaMercanciaService::emitir($e, $this->concepto_rol);
