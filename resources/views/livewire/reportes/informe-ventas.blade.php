...
<div class="p-4 md:p-6 space-y-6">
    ...
    <section class="rounded-3xl bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 shadow-xl p-6">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          ...
          {{ $esCompra ? 'bg-amber-100 text-amber-700' : 'bg-indigo-100 text-indigo-700' }}">
                ...
                <span
                    class="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-200 text-xs font-semibold">
    ...
    <section class="grid grid-cols-2 md:grid-cols-6 gap-4">
        <div class="rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-4 shadow">
            ...
            <div class="text-xl font-extrabold text-gray-900 dark:text-white">{{ number_format($totalFacturas ?? 0) }}
            </div>
        </div>
        ...
        <div class="rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-4 shadow">
            ...
            <div class="text-xl font-extrabold text-gray-900 dark:text-white">
                ${{ number_format($totalFacturado ?? 0, 0, ',', '.') }}</div>
        </div>
        ...
        <div class="rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-4 shadow">
            ...
            <div class="text-xl font-extrabold text-gray-900 dark:text-white">
                ${{ number_format($totalContado ?? 0, 0, ',', '.') }}</div>
        </div>

        <div class="rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-4 shadow">
            <div class="text-xs text-gray-500">Crédito</div>
            <div class="text-xl font-extrabold text-gray-900 dark:text-white">
                ${{ number_format($totalCredito ?? 0, 0, ',', '.') }}</div>
        </div>
    </section>
        ...
        <div class="grid grid-cols-1 md:grid-cols-6 gap-4">
            ...
            <div>
                ...
                <select wire:model.live="estadoFiltro"
                    class="w-full h-10 px-3 rounded-xl border dark:bg-gray-800 dark:text-white">
            ...
            <div>
                ...
                <select wire:model.live="tipoPagoFiltro"
                    class="w-full h-10 px-3 rounded-xl border dark:bg-gray-800 dark:text-white">
                    <option value="todos">Todos</option>
            ...
            <div class="flex flex-col">
                ...
                <div class="relative">
                    <input type="text" list="terceros_list" wire:model.debounce.500ms="filtroCliente"
    ...
    {{-- ===================== TABLA ===================== --}}
    <section
        class="rounded-2xl bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 shadow overflow-hidden">
                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-800">
                            ...
                            <td class="px-4 py-3">
                                @if ($esCompra)
                                    ...
                                    @if ($f->socioNegocio?->nit)
                                        <span class="text-xs text-gray-500">({{ $f->socioNegocio->nit }})</span>
                                    @endif
                                @else
                                    {{ $f->cliente?->razon_social ?? '—' }}
                                    @if ($f->cliente?->nit)
                                        <span class="text-xs text-gray-500">({{ $f->cliente->nit }})</span>
                                    @endif
                                @endif
                            </td>
